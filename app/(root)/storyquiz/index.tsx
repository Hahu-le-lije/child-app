    ...
    <GameLayout title="Story Quiz">
      <View style={styles.container}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.card} onPress={() => router.push(`/(root)/storyquiz/${item.id}`)}>
              <Text style={styles.title}>{item.title || `Level ${item.level_number}`}</Text>
              <Text style={styles.subtitle}>{item.description || 'Read and answer questions'}</Text>
            </TouchableOpacity>
          )}
        />
      </View>
